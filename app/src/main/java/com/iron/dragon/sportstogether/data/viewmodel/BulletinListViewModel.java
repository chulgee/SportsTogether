...
import com.iron.dragon.sportstogether.BR;
...
/**
 * Created by P16018 on 2016-12-23.
 */
...
public class BulletinListViewModel extends BaseObservable {
    ...
    public String numberOfUsersLoggedIn;
    ...
    public BulletinListViewModel(BulletinListActivity bulletinListActivity_, int extra_sports) {
        mActivity = bulletinListActivity_;
    ...
    private File createNewFile(String prefix) {
        ...
        if (!newDirectory.exists()) {
            if (newDirectory.mkdir()) {
                Logger.d(newDirectory.getAbsolutePath() + " directory created");
            }
        }
        ...
        if (requestCode == REQ_CODE_PICK_PICTURE) {
            ...
            if (resultCode == Activity.RESULT_OK) {
                if (mCropImagedUri == null) {
                    mActivity.showErrorCaptureImage(R.string.capture_error);
                    return;
                }
            ...
            if (resultCode == Activity.RESULT_OK) {
                ...
                mActivity.addImageView(getDownsampledBitmap(mCropImagedUri, 150, 150));


            } else {
                mActivity.showErrorCropImage(R.string.crop_error);
            }
        }
    }

    private BitmapFactory.Options getBitmapDimensions(ContentResolver contentResolver, Uri uri) throws IOException {
    ...
    private Bitmap getDownsampledBitmap(Uri uri, int targetWidth, int targetHeight) {
        Bitmap bitmap = null;
        try {
            ContentResolver ctr = mActivity.getContentResolver();
    ...
    private Bitmap downsampleBitmap(ContentResolver ctr, Uri uri, int sampleSize) throws IOException {
            ...
            public void onStateChanged(View bottomSheet, int newState) {
                ...
                switch (newState) {
                    case BottomSheetBehavior.STATE_COLLAPSED:
                        mActivity.FabUpdateVisible(View.VISIBLE);
                        break;
                    case BottomSheetBehavior.STATE_DRAGGING:
                        Log.e("Bottom Sheet Behaviour", "STATE_DRAGGING");
                        break;
                    case BottomSheetBehavior.STATE_EXPANDED:
                        mActivity.FabUpdateVisible(View.GONE);
